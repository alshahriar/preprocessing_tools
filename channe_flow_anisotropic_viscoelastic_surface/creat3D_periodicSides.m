...
if(strcmp(geometry,'rectangle'))
    ...
    nPlaneInZ = 79;
    nz = nPlaneInZ;
    leftSide = 0; rightSide = 4*pi; % range of flexbile part - it will not be exactly these points - but it will be very close
    frontSide = 0; backSide = 2*pi;    
    end
...
ze = zs+nPlaneInZ*dz;
zz = linspace(zs,ze,dz);
% adjusting so that flexible panel exactly starts at leftSide
dz = (frontSide-zs)/floor((frontSide-zs)/dz);
newTempZ = zs:dz:ze;
ze = newTempZ(end);
...
for iN = 1:nPointsForUnstructured
    for k = 1:nzT
        for j = 1: nyT
            for i = 1: nxT
                if(pointsP1(iN,1)==xTahoe(i) && pointsP1(iN,2)==yTahoe(j) && pointsP1(iN,3)==zTahoe(k))
                    flag = 1;
                    break;
                else
                    flag = 0;
...
%nKBC = 2*nyT*nzT + 2*(nxT-2)*nyT + (nxT*nzT - 2*nzT - 2*(nxT-2));
  nKBC =   (nzT)*nyT ...  % left side all
         + (nzT)*nyT ...  % right side all
         + (nzT)*(nxT-2);   % bottom side without left, right boundary;
...
KBC2FEM(1:countKBCL+countKBCR+countKBCBot) = [indexKBC_L indexKBC_R indexKBC_Bot];
...
% Periodic front and back side

nPrBC = (nxT-2)*(nyT-1) ...  % front side excluding left right bottom
      + (nxT-2)*(nyT-1); % back side excluding left right bottom
...
PrBC2FEM(1:countPrBCF+countPrBCB) = [indexPrBC_F indexPrBC_B];
...
fid = fopen('TahoePrBCnodesBody1.dat','w');
for i = 1 : countPrBCF

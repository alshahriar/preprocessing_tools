...
if(strcmp(geometry,'rectangle'))
    ...
    if(1)
        ...
        ye = ys + lengthY;
        nx = 180;
        leftSide = 0; rightSide = 6.12; % range of flexbile part - it will not be exactly these points - but it will be very close
        frontSide = 0; backSide = 3.1944;    
        nLayerMarker = 2;
    end
    
    x = linspace(xs,xe,nx);
    y1  = linspace(ys,ys+h1,nn1);
    y2 = linspace(ys+h1,ys+h1+h2,nn2);
    y3 =  linspace(ys+h1+h2,ys+h1+h2+h3,nn3);
    y = [y1 y2(2:nn2) y3(2:nn3)];
    dx = x(2) - x(1);
    
    % adjusting so that flexible panel exactly starts at leftSide
    dx = (leftSide-xs)/floor((leftSide-xs)/dx);
    nxTahoe = ceil((rightSide-leftSide)/dx);
...
% 
leftMostOld = leftMost;
leftMost = nPoints/2 + rightMost;
rightMost = nPoints/2 + leftMostOld;

% leftMost = leftMost +2;
% rightMost = rightMost + 2;

disp("Left most : right most = " + [newX(leftMost) newX(rightMost)])
disp("frontMost : frontMost = " + [z(frontMost) z(backMost)])

% imposing periodic forces
% xlines (front and back)
% From front to back
xLineMarkerFront = zeros(leftMost-rightMost+1,1);
xLineMarkerBack = zeros(leftMost-rightMost+1,1);
zLineMarkerLeft = zeros(backMost-frontMost+1,1);
zLineMarkerRight = zeros(backMost-frontMost+1,1);
count = 0;
k = frontMost;
for i = leftMost:-1:rightMost
    count = count + 1;
    xLineMarkerFront(count) = (k-1)*nPoints+i;
end
% From back to front
count = 0;
k = backMost;
for i = leftMost:-1:rightMost
    count = count + 1;
    xLineMarkerBack(count) = (k-1)*nPoints+i;
end
% From left to right
count = 0;
i = leftMost;
for k=frontMost:backMost
    count = count + 1;
    zLineMarkerLeft(count) = (k-1)*nPoints+i;
end
% From right to left
count = 0;
i = rightMost;
for k=frontMost:backMost
    count = count + 1;
    zLineMarkerRight(count) = (k-1)*nPoints+i;
end
figure(1)
hold on
plot(pointsP1(xLineMarkerFront,1),pointsP1(xLineMarkerFront,3))
plot(pointsP1(xLineMarkerBack,1),pointsP1(xLineMarkerBack,3))
plot(pointsP1(zLineMarkerLeft,1),pointsP1(zLineMarkerLeft,3))
plot(pointsP1(zLineMarkerRight,1),pointsP1(zLineMarkerRight,3))
%%
disp("data for CCNS: " + leftMost + " " + rightMost + " " + frontMost + ...
    " "+ backMost + " " + nPoints + " 3")

				for iLayer = 1:nLayerMarker
					for i = leftMost:-1:rightMost
						k = backMost - iLayer;
						indexSender =  + (k-1)*nPoints+i;
						k = frontMost - iLayer + 1;
						indexReceiv = (k-1)*nPoints+i;
					end
					% From front to back
					for i = leftMost:-1:rightMost
						k = frontMost+iLayer;
...
xTahoe = newX(leftMost:-1:rightMost);
yTahoe = y;
...
disp("nlength in CCNS.inp: " + 3*nFBC)
...
min_NnZDR = 2; % min number of line DR
min_nzDR = 1; % min size of  each DR
...
disp("Tahoe node and elements: " + Nn + "  " + Ee)
